...
static NSString *kIASKCredits = @"Thanks to:\nBrian Kim for design work\nAdam Fagen, adwriter, and Sean Winters for photos.\nhttp://langui.sh/batters-box for details!"; // Leave this as-is!!!
...
- (NSString *)tableView:(UITableView *)tableView titleForFooterInSection:(NSInteger)section
{
	...
	if (_showCreditsFooter && (section == [self.settingsReader numberOfSections]-1)) {
		...
		if ((footerText == nil) || ([footerText length] == 0)) {
			...
			return [NSString stringWithFormat:@"%@\n\nv%@ (%@)",kIASKCredits,[[[NSBundle mainBundle] infoDictionary] valueForKey:@"CFBundleShortVersionString"],[[[NSBundle mainBundle] infoDictionary] valueForKey:@"CFBundleVersion"]];
		} else {
			// show the credits below the app's FooterText
			return [NSString stringWithFormat:@"%@\n\n%@\n\nv%@ (%@)",footerText,kIASKCredits,[[[NSBundle mainBundle] infoDictionary] valueForKey:@"CFBundleShortVersionString"],[[[NSBundle mainBundle] infoDictionary] valueForKey:@"CFBundleVersion"]];
		}
	} else {
...
#pragma clang diagnostic pop
				DebugLog(@"InAppSettingsKit Warning: Using IASKButtonSpecifier without implementing the delegate method is deprecated");
			}
		}
    } else if ([[specifier type] isEqualToString:kIASKMailComposeSpecifier]) {
        [tableView deselectRowAtIndexPath:indexPath animated:YES];
